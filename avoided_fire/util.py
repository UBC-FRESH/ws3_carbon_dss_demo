...
##########################################################
# Implement a priority queue heuristic fire scheduler_null
##########################################################

def schedule_fire_areacontrol_null(fm, period=1, acode='fire', util=0.85, 
    ...
    for mask in target_masks: 
        # if verbose > 0:
        #     print('calling areaselector', period, acode, target_area, mask)
        fm.areaselector.operate(period, acode, target_area= 0, mask=mask, verbose=verbose)
    sch = fm.compile_schedule()
    return sch


##########################################################
# Implement a priority queue heuristic harvest scheduler_null
##########################################################

def schedule_harvest_areacontrol_null(fm, period=1, acode='harvest', util=0.85, 
                                 target_masks=None, target_areas=None, target_scalefactors=None,
                                 mask_area_thresh=0.,
                                 verbose=0):
    #fm.reset_actions()
...
##########################################################
# Implement a priority queue heuristic harvest scheduler_asap
##########################################################

def schedule_harvest_areacontrol_asap(fm, period=1, acode='harvest', util=0.85, 
                                 target_masks=None, target_areas=None, target_scalefactors=None,
                                 mask_area_thresh=0.,
                                 verbose=0):
    #fm.reset_actions()
    ...
    for mask in target_masks: 
        # if verbose > 0:
        #     print('calling areaselector', period, acode, target_area, mask)
        fm.areaselector.operate(period, acode, target_area= 21940.231286319417, mask=mask, verbose=verbose)
    sch = fm.compile_schedule()
    return sch
...
def compile_scenario(fm):
    oha = [fm.compile_product(period, '1.', acode='fire') for period in fm.periods]
    ohv = [fm.compile_product(period, 'totvol * 0.85 ', acode='fire') for period in fm.periods]
...
def plot_scenario(df):
    ...
    ax[0].set_title('Burned area (ha)')
    ax[1].bar(df.period, df.ohv)
    ax[1].set_ylim(0, None)
    ax[1].set_title('Burned volume (m3)')
...
def plot_results(fm):
    ...
    ax[0].set_ylabel('Burned area')
    ax[0].bar(df.period, df.ha)
    ax[1].set_ylabel('Burned volume')
    ax[1].bar(df.period, df.hv)
    ax[2].set_ylabel('Burned volume:area ratio')
